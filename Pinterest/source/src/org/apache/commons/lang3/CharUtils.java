...
public class CharUtils
{
    ...
    public static int toIntValue(char c)
    {
        if (!isAsciiNumeric(c))
        {
            throw new IllegalArgumentException((new StringBuilder("The character ")).append(c).append(" is not in the range '0' - '9'").toString());
        } else
        {
            return c - 48;
        }
    ...
    public static String unicodeEscaped(char c)
    {
        if (c < '\020')
        {
            return (new StringBuilder("\\u000")).append(Integer.toHexString(c)).toString();
        }
        if (c < '\u0100')
        {
            return (new StringBuilder("\\u00")).append(Integer.toHexString(c)).toString();
        }
        if (c < '\u1000')
        {
            return (new StringBuilder("\\u0")).append(Integer.toHexString(c)).toString();
        } else
        {
            return (new StringBuilder("\\u")).append(Integer.toHexString(c)).toString();
        }
    }
