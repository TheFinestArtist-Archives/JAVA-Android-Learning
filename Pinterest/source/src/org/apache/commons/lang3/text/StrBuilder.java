    ...
    implements Appendable, CharSequence
{
    ...
    private StrBuilder replaceImpl(StrMatcher strmatcher, String s, int i, int j, int k)
    {
        if (strmatcher != null && size != 0)
        {
            ...
            while (k < l && j != 0) 
            {
                int k1 = strmatcher.isMatch(ac, k, i, l);
                if (k1 > 0)
                {
                    replaceImpl(k, k + k1, k1, s, i1);
                    k1 = (l - k1) + i1;
                    l = (k + i1) - 1;
                    if (j > 0)
                    {
    ...
    public StrBuilder appendFixedWidthPadLeft(Object obj, int i, char c)
    {
        if (i > 0)
        {
            ...
            } else
            {
                ...
                ((String) (obj1)).getChars(0, k, buffer, l + size);
            }
            size = size + i;
        }
        return this;
...
label1:
            {
                ...
                k = j;
                if (strmatcher.isMatch(ac, j, i, l) > 0)
                {
                    break label0;
                }
                j++;
...
label1:
            {
                ...
                j = k;
                if (strmatcher.isMatch(ac, k, 0, i + 1) > 0)
                {
                    break label0;
                }
                j = k - 1;
...
_L3:
    ...
    private class StrBuilderReader extends Reader
    {
    ...
    private class StrBuilderTokenizer extends StrTokenizer
    {
    ...
    private class StrBuilderWriter extends Writer
    {
