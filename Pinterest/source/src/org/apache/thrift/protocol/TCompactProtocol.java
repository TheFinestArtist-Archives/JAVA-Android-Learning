...
public class TCompactProtocol extends TProtocol
{
    ...
    private void checkReadLength(int i)
    {
        if (i < 0)
        {
            throw new TProtocolException((new StringBuilder("Negative length: ")).append(i).toString());
        }
        if (maxNetworkBytes_ != -1L && (long)i > maxNetworkBytes_)
        {
            throw new TProtocolException((new StringBuilder("Length exceeded max allowed: ")).append(i).toString());
        } else
        {
            return;
        }
