...
label0:
    ...
    public void animateToVisibility(int i)
    {
        if (i == 8)
        {
            ViewCompat.animate(mToolbar).alpha(0.0F).setListener(new _cls2());
        } else
        if (i == 0)
        {
            ViewCompat.animate(mToolbar).alpha(1.0F).setListener(new _cls3());
            return;
        }
    }
...
_L6:
        throw new IllegalArgumentException((new StringBuilder("Invalid navigation mode ")).append(i).toString());
_L4:
...
_L9:
    ...
    private class _cls3 extends ViewPropertyAnimatorListenerAdapter
    {
