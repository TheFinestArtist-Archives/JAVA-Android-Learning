    ...
    implements Parcelable
{
    ...
    public final BackStackRecord instantiate(FragmentManagerImpl fragmentmanagerimpl)
    {
        ...
        for (int i = 0; i < mOps.length;)
        {
            ...
            {
                Log.v("FragmentManager", (new StringBuilder("Instantiate ")).append(backstackrecord).append(" op #").append(k).append(" base fragment #").append(mOps[j]).toString());
            }
            ai = mOps;
            i = j + 1;
            j = ai[j];
            ...
            if (i1 > 0)
            {
                ...
                do
                {
                    ...
                    if (FragmentManagerImpl.DEBUG)
                    {
                        Log.v("FragmentManager", (new StringBuilder("Instantiate ")).append(backstackrecord).append(" set remove fragment #").append(mOps[j]).toString());
                    }
