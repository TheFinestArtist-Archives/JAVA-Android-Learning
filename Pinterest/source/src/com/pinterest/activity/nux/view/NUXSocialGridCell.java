...
public class NUXSocialGridCell extends RelativeLayout
{
    ...
    private boolean touchEvent(View view, ImageView imageview, MotionEvent motionevent)
    {
        ...
        if (_dimOnTouch)
        {
            if (motionevent.getAction() == 0 && !_isSelected)
            {
                imageview.setColorFilter(getResources().getColor(0x7f090049));
                _isSelected = true;
                return true;
            }
    ...
    public void init()
    {
        inflate(getContext(), 0x7f0300be, this);
        _checked = false;
        _image = (ProportionalImageView)findViewById(0x7f0b0039);
        _image.setOval(true);
        _name = (PTextView)findViewById(0x7f0b012c);
        _isFirstCell = false;
    ...
    public void setIsFirstCell(boolean flag, boolean flag1)
    {
        ...
        if (_isFirstCell)
        {
            setNameText(com.pinterest.kit.application.Resources.string(0x7f0e031d));
            ImageView imageview;
            if (flag1)
            {
                _image.setImageDrawable(getResources().getDrawable(0x7f020186));
            } else
            {
                _image.setImageDrawable(getResources().getDrawable(0x7f020187));
            }
        }
