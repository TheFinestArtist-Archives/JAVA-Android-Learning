...
_L2:
        ...
        do
        {
            ...
            type = matchresult.group();
            notificationsubject = notificationtemplate.getSubject(type.substring(1, type.length() - 1));
            type = notificationsubject.getType();
            s1 = notificationsubject.getValue();
            spannablestringbuilder = s.replace(matchresult.start(), matchresult.end(), s1);
        ...
        {
        default:
            (new StringBuilder("unknown subjectType in template:")).append(type);
            s = spannablestringbuilder;
...
_L3:
        spannablestringbuilder.setSpan(new _cls3(), matchresult.start(), matchresult.start() + s1.length(), 17);
        s = spannablestringbuilder;
...
_L1:
        (new StringBuilder("unknown modeltype:")).append(datatype);
        return;
_L2:
