...
public class FilterDialogView extends FrameLayout
{
    ...
    {
        return i / ((int)Resources.dimension(0x7f0a00d5) + (int)Resources.dimension(0x7f0a00d4));
    }

    private Animation getScaleAnimation(float f, float f1, float f2, float f3, boolean flag)
    {
        float f5 = (float)_guideXOffset + getResources().getDimension(0x7f0a00da) / 2.0F;
        float f4 = f5;
        if (_position == 0)
        {
            f4 = f5 + getResources().getDimension(0x7f0a0098);
        }
        ScaleAnimation scaleanimation = new ScaleAnimation(f, f1, f2, f3, f4, 0.0F);
        scaleanimation.setFillAfter(true);
        scaleanimation.setFillEnabled(true);
        scaleanimation.setDuration(getResources().getInteger(0x7f0c0003));
        scaleanimation.setInterpolator(new OvershootInterpolator(2.0F));
        AnimationSet animationset = new AnimationSet(true);
        animationset.addAnimation(scaleanimation);
        if (flag)
        {
            animationset.addAnimation(AnimationUtils.loadAnimation(getContext(), 0x7f04000c));
            return animationset;
        } else
        {
            animationset.addAnimation(AnimationUtils.loadAnimation(getContext(), 0x7f040010));
            return animationset;
        }
    }
    ...
    private void performFilter(FilterOption filteroption)
    {
        ...
        } else
        {
            ...
            guidedsearchevent.setActionFilter(new SearchFilter(filteroption.getId(), _filterType, filteroption.getDisplayName(), filteroption.getTypeValue(), null, null, null));
    ...
    protected void init()
    {
        LayoutInflater.from(getContext()).inflate(0x7f03005b, this, true);
        ButterKnife.a(this);
    ...
    {
        _spacerTopHeight = (int)(Resources.dimension(0x7f0a00e0) + (float)Constants.MARGIN);
        ...
        implements Runnable
    {
        ...
        public void run()
        {
            ...
            if (_position == 0)
            {
                i = (int)((float)j + Resources.dimension(0x7f0a0098) / 2.0F);
            }
            _anchor.setX(i);
        }
