...
public class BoardGridAdapter extends PinterestBaseAdapter
{
    ...
    public void followAllBoards(boolean flag)
    {
        followAllTopBoards(getCount(), flag);
    }

    public void followAllTopBoards(int i, boolean flag)
    {
        if (_dataSource == null)
        {
            return;
        ...
        for (i = 0; i < j; i++)
        {
            ((Board)getItem(i)).setFollowing(Boolean.valueOf(flag));
        }
    ...
    public void overrideAllBoardsFollow(User user)
    {
        ...
        boolean flag = user.getFollowing();
        user = user.getUid();
        int j = getCount();
        for (int i = 0; i < j; i++)
        {
    ...
    public boolean restoreInstanceState(Bundle bundle)
    {
        ...
        {
            _dataSource = null;
            CrashReporting.leaveBreadcrumb("BoardGridAdapter.restoreInstanceState: not a BoardFeed");
            CrashReporting.logHandledException(new RuntimeException("BoardGridAdapter.restoreInstanceState: not a BoardFeed"));
        }
        return flag;
    }
